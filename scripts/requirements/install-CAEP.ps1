...
Write-Host 'Searching for @ca npm packages...'
$npmList = npm list -g --depth=0
if ( ($npmList -like '*@ca/cli*') -and ($npmList -like '*@ca/generator-scarface*') ) {
 Write-Host '@ca/cli and @ca/generator-scarface are installed npm packages.'
} else {
	Write-Host 'npm packages @ca/cli and @ca/generator-scarface not found!'
    return @($true, 'KO')
}

Write-Host 'Searching for ca plugins...'
$npmPlugin = ca plugins
if ( $npmPlugin -like '*@ca/cli-plugin-scarface*' ) {
	Write-Host 'ca plugins: @ca/cli-plugin-scarface found!'
    return @($true, 'OK')
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU8bs3sVqUFq9ezB1q/l/BFo/U
# Lg6ggh6lMIIFOTCCBCGgAwIBAgIQDue4N8WIaRr2ZZle0AzJjDANBgkqhkiG9w0B
...
# MQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUOEhjmKZp/ulDy5n7Eoz0
# lKXQC+EwJAYKKwYBBAGCNwIBDDEWMBSgEoAQAEMAQQAgAFQAbwBvAGwAczANBgkq
# hkiG9w0BAQEFAASCAQBdVooIlel5bJRzZBH/bsOJZv32axa8VqozsWMDkrxQJ3V2
# QtdWXfvXowN7hljHGoTtZ9Dz4/8o9zxPts5BVf4oywp1F/K5j0zwmjMQiIt77i56
# t8Dukdj78Ihn65LqJ69iqq2fHtzjuE4o4MxeOGaM+EAU0NQjU+hUYcIh6t9f0V9J
# slpsBLGTrQvLOb4hvDjkeQMqWuHkIvEypQFyva62CLQ9KDise/4B1plRgJ21xk5c
# FbkyTZ5ZpBJNqZOWzd69PeXT40aHakS+jWBW64zIE8h081FeAave1jvwBwpN/GVT
# Q7d07atJuMV2clo8OQpeTytMEqsq8N3c/MYt6B+YoYIDTDCCA0gGCSqGSIb3DQEJ
...
# CwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMzAyMTcwOTM0MjdaMD8GCSqG
# SIb3DQEJBDEyBDB0WO3OiL13UIK6/B0k4PwgM/vlDeiHsgtcWLpMViPoAPN9WTeb
# xklJL/OIZF61OSwwDQYJKoZIhvcNAQEBBQAEggIASRhWD04GBZyU2qEWgkYNO2nC
# M+RbLU7LbpyTlL1nhcv6O3KiyuzIHiGqVwKh6p+xV8Lr04yggsCwLV+zeuGy2Un5
# bt6+yyzAdeEENedE4N60pWKrB8ebPlfJR31+DLQwWNeR/IfXsVflKEHA+d8GoelC
# Dv+LIghdx8wZByih3sHzc4hC3vhapefUW008uj2kkUdNkpf2lXtVR1EebRQG7uie
# PoA31qocJqSETnL3lc7bPHYmgZvfTGTBI541r+jM6jYpo1sVe6EJ9zl8PNnaWPzF
# tdo7TP9AfV3UyTfqrueU0ynGTB9xIbUVLfusW4RKW1c0i76wqHSPveoWHcuMp8TF
# PWnfsiDSaqm948B56M9NC590wrW1VhhXBnVjRYsIctkQ8cy4x3d0B2PFapuOagn9
# HWmImzZis/9lM1AwwwJ7Yn/KC+vuwbyitE3hD2XRCDger4yXXXskzpWxpAmSbjd/
# 5++SzoKuIQmotMNEnONOq7sB+dypU7A7E/Fidt8YGFGf4bw0iT08IVYo3aApO17r
# evQGnvV59InWdDSgE6zGFHxV3e3FJiI/6bixcRuP4sg5hiHl7XIQnQScWlPiD8zU
# lyYnFwA0erRAVDkOwMuD4+KusnE20iqGX8D9o27/f5SS0UuLCZC4KPlA+nErW29m
# +4K7OmB0H2rgAR4AqPQ=
